...
| 3 | Port `synth/placer.py` + `synth/router.py` + `synth/auto_router.py` (+ `synthesizer.py`, `netlist.py`, `port_resolver.py`, `constraints.py`, `euler.py`, `geo/`) | in progress | Foundation + placement + router (7 of 13 style handlers) + skeleton synthesizer ported. End-to-end inverter GDS now produced from `Synthesizer(rules).synthesize(load_template("inverter"), routing_specs=…)`. Remaining: `auto_router` (~874 LoC, emits routing specs from NetGraph), the 6 advanced router handlers (gate-to-drain, cross-couple, vertical/cross-row buses, etc.), and full DRC/LVS hooks on the synthesizer. `geo/` (1948 LoC of RL-flavoured agent code) deferred to step 6. |
